...
function simulateTestProbe(connectionConfig, addresses) {
  ...
  const results = createProbeResultTemplate(addresses).map((entry, index) => ({
    ...
    value: formatValueForDisplay(simulateTestValue(entry.address, index, cycle))
  }));
...
function normalizeRootConfig(config) {
  ...
  const normalized = {
    ...
    entities: rawEntities.filter((entity) => isPlainObject(entity)).map((entity) => ({ ...entity })),
    extras: {}
  };
...
function probePlc(connectionConfig, addresses) {
  ...
  return new Promise((resolve) => {
    ...
    const finish = (resultUpdate) => {
      ...
      const providedConnection = Object.prototype.hasOwnProperty.call(resultUpdate || {}, 'connection')
        ? resultUpdate.connection
        : null;
      const connection = providedConnection
        ? providedConnection
        : baseResult.connection
            ? { ...baseResult.connection, mode: 'live', lastScan: new Date().toISOString() }
            : null;
...
const server = http.createServer(async (req, res) => {
    ...
    fs.readFile(resolved, 'utf-8', (error, content) => {
      if (error) {
        if (error.code === 'ENOENT') {
          sendJson(res, 200, { file, data: normalizeRootConfig(cloneStandardConfig()) });
          return;
        }
  ...
  if (method === 'GET' && url.pathname === '/api/plc/test-mode') {
    ...
    sendJson(res, 200, {
      ...state,
      message: state.enabled ? 'Testmodus ist aktiv.' : 'Testmodus ist deaktiviert.'
    });
    return;
  }
  ...
  if (method === 'POST' && url.pathname === '/api/plc/test-mode') {
    try {
      ...
      sendJson(res, 200, {
        ...state,
        message: state.enabled ? 'Testmodus aktiviert.' : 'Testmodus deaktiviert.'
      });
    } catch (error) {
      sendJson(res, 400, { message: 'Testmodus konnte nicht aktualisiert werden.' });
    }
    return;
  }
